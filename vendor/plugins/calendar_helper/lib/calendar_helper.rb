...
module CalendarHelper
  ...
  #   :month_name_text   => nil               # The heading text, default the name of the current month
  ...
  def calendar(options = {}, &block)
    ...
    if options[:previous_month_text] or options[:next_month_text]
      cal << %(<th colspan="2" class="previousMonth">#{options[:previous_month_text]}</th>)
      colspan=3
    else
      colspan=7
    end
    cal << %(<th colspan="#{colspan}" class="#{options[:month_name_class]}">#{options[:month_name_text] or Date::MONTHNAMES[options[:month]]}</th>)
    cal << %(<th colspan="2" class="nextMonth">#{options[:next_month_text]}</th>) if options[:next_month_text]
    ...
    first.upto(last) do |cur|
      ...
      cell_attrs[:class] += " borderEndDay" if cur.wday == last_weekday 
    ...
    (last + 1).upto(beginning_of_week(last + 7, first_weekday) - 1)  do |d|
      cal << %(<td class="#{options[:other_month_class]})
      cal << " weekendDay" if weekend?(d)
      cal << " borderEndDay" if d.wday == last_weekday 
